...
| Contrôle à l'entrée | choix de l'événement **puis de l'activité** (ou entrée générale) ; bascule **Entrée / Sortie** si l'événement suit le flux ; compteurs live (entrées, sorties, présents, ré-entrées) ; scan caméra → VALIDE / REFUSÉ / INVALIDE | `GET /checkins/events` (+ `/{id}/activities`), `POST /checkins/scan` (`activityId`, `sens`), `GET /events/{id}/checkin-stats` |
