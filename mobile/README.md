...
| Contrôle à l'entrée | choix de l'événement **puis de l'activité** (ou entrée générale) ; bascule **Entrée / Sortie** si l'événement suit le flux (y compris par activité) ; compteurs live (entrées, sorties, présents, ré-entrées) pour l'entrée générale ou l'activité choisie ; scan caméra → VALIDE / REFUSÉ / INVALIDE | `GET /checkins/events` (+ `/{id}/activities`), `POST /checkins/scan` (`activityId`, `sens`), `GET /events/{id}/checkin-stats?activityId=` |
